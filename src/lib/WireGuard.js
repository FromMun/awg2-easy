...
module.exports = class WireGuard {
  ...
  async getConfig() {
    if (!this.__configPromise) {
      ...
      await Util.exec(`awg-quick down ${WG_INTERFACE}`).catch(() => {});
      await Util.exec(`awg-quick up ${WG_INTERFACE}`).catch((err) => {
      ...
      // await Util.exec('iptables -A FORWARD -i awg0 -j ACCEPT');
      // await Util.exec('iptables -A FORWARD -o awg0 -j ACCEPT');
...
${client.preSharedKey ? `PresharedKey = ${client.preSharedKey}\n` : ''
  ...
  async __syncConfig() {
    ...
    await Util.exec(`awg syncconf ${WG_INTERFACE} <(awg-quick strip ${WG_INTERFACE})`);
    debug('Config synced.');
  }
  ...
  async getClients() {
    ...
    const dump = await Util.exec(`awg show ${WG_INTERFACE} dump`, {
      log: false,
    });
    dump
      .trim()
...
Endpoint = ${WG_HOST}:${WG_CONFIG_PORT}`;
  ...
  async getMetrics() {
    ...
    returnText += `wireguard_configured_peers{interface="${WG_INTERFACE}"} ${Number(wireguardPeerCount)}\n`;

    returnText += '\n# HELP wireguard_enabled_peers\n';
    returnText += '# TYPE wireguard_enabled_peers gauge\n';
    returnText += `wireguard_enabled_peers{interface="${WG_INTERFACE}"} ${Number(wireguardEnabledPeersCount)}\n`;

    returnText += '\n# HELP wireguard_connected_peers\n';
    returnText += '# TYPE wireguard_connected_peers gauge\n';
    returnText += `wireguard_connected_peers{interface="${WG_INTERFACE}"} ${Number(wireguardConnectedPeersCount)}\n`;
